...
update msg model =
    case msg of
        ...
        GotCheckedLocations locationIds ->
            let
                _ = Debug.log "GotCheckedLocations" locationIds
            in
            ( { model
                | pendingSolvedBlocks =
                    Set.union
        ...
        UnlockSelectedBlockPressed ->
            List.foldl
                (andThen << \block -> (unlockBlock True ( block.startRow, block.startCol ))
                )
                ( model
                , Cmd.none
                )
...
updateStateSolvedArea triggerAnimations cellAreas toId ( row, col ) model =
    let
        _ = Debug.log "Updating solved area at:" ( row, col )
        cells : List ( Int, Int )
        cells =
    ...
    case model.localGameSave of
        Just save ->
            Html.div
                ...
                -- TODO: Add note about resuming online games
                ]
