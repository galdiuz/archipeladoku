...
class ArchipeladokuBoard extends HTMLElement {
    ...
    set data(value: BoardData) {
        ...
        if (!this.discoTrap && value.discoTrap) {
            ...
            this.animations.delete('disco-trap')
        }
    ...
    startDiscoTrapAnimation() {
        ...
        const numSpotlights = Math.floor(this.clientWidth / 100)

        for (let i = 0; i < numSpotlights; i++) {
            this.discoTrapSpotlights.push({
                originXRatio: 0.05 + 0.9 * (i / (numSpotlights - 1)),
                length: Math.random() * 300 + 200,
                widthAngle: Math.random() * (Math.PI / 8) + (Math.PI / 16),
                currentAngle: -Math.PI / 2,
                initialAngle: -Math.PI / 2,
                sweepRange: Math.PI / 8,
                sweepSpeed: Math.random() * 0.0002,
        ...
        this.addAnimation({
            ...
            onUpdate: (t: number) => {
                for (const spotlight of this.discoTrapSpotlights) {
                    spotlight.timeOffset += spotlight.sweepSpeed
                    spotlight.currentAngle = Math.sin((t + spotlight.timeOffset) * Math.PI * 2)
                        * spotlight.sweepRange
                        + spotlight.initialAngle
                }
                // console.log('disco trap progress:', t)
            }
        })
    }
    ...
    renderDiscoTrapSpotlight(
        ...
        const startY = (this.canvas.height - this.viewport.y + 25) / this.viewport.scale

        const length = spotlight.length / this.viewport.scale
        const leftAngle = spotlight.currentAngle - (spotlight.widthAngle / 2)
        const rightAngle = spotlight.currentAngle + (spotlight.widthAngle / 2)
        ...
        gradient.addColorStop(0, `hsl(${hue} 100 60 / 0.6)`)
        gradient.addColorStop(0.5, `hsl(${hue} 100 60 / 0.2)`)
        gradient.addColorStop(1, `hsl(${hue} 100 60 / 0)`)
        ctx.fillStyle = gradient
        ctx.fill()
    }
